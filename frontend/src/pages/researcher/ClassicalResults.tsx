...
import { fetchClassical } from '../../api';

const ClassicalResults = () => {
  const [data, setData] = useState<any>(null);

  useEffect(() => {
    fetchClassical().then(setData);
  }, []);
  ...
  return (
    <div>
      <h1>Classical ML Results</h1>
      <h3 style={{color: 'var(--text-secondary)', marginBottom: '32px'}}>Model: {data.model_used}</h3>
...
export default ClassicalResults;
